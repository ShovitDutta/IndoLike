...
interface ArtistCardProps {
  image?: string;
  name?: string;
  id?: string; // Although not directly used in the component's logic, it's passed in the original
}
        ...
        <div className="relative aspect-square w-full">
          <div className="overflow-hidden rounded-2xl bg-card/40 backdrop-blur-sm">
            <motion.div className="w-full h-full" whileHover={{ scale: 1.15 }} transition={{ duration: 0.4 }}>
              {image && <img src={image} alt={name} className="h-full w-full object-cover transition-all duration-300" />}
            </motion.div>
          </div>
          <motion.div
