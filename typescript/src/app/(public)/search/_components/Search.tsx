...
import ArtistCard from "@/components/cards/artist"; // Assuming ArtistCard will be created in TSX
...
export default function Search({ params }: SearchProps) {
  ...
  const [artists, setArtists] = useState<SongResult[]>([]);
  ...
  const getSongs = async () => {
    try {
      ...
      if (data?.data?.results) {
        ...
        setArtists(data.data.results); // Assuming artists data is also in song results
      }
    } catch (error) {
              ...
              {albums.length > 0 ? (
                albums.map((album, index) => (
                  <motion.div key={album.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * index }}>
                    <AlbumCard
                      lang={album.language}
                      desc={album.description || undefined} // Use undefined for optional props
                      id={`album/${album.id}`}
            ...
            {artists.length > 0 ? (
              <motion.div className="flex gap-4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }}>
                {[...new Set(artists.map(a => a.artists.primary[0]?.id))].map((id, index) => {
                  ...
                  if (!artist) return null; // Handle case where artist is not found

                  const artistImage = artist.image?.[2]?.url ||
                    `https://az-avatar.vercel.app/api/avatar/?bgColor=0f0f0f0&fontSize=60&text=${artist.name.split("")[0]?.toUpperCase() || "U"}`;
