...
import { Credenza, CredenzaBody, CredenzaContent, CredenzaHeader, CredenzaTitle, CredenzaTrigger } from "@/components/ui/credenza"; // Assuming Credenza components will be created in TSX
import React from "react";

// Define types for the fetched song search results (simplified)
          ...
          <CredenzaHeader>
            <CredenzaTitle className="text-left flex gap-2">
              <Input
                ...
                value={query}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQuery(e.target.value)} // Explicitly type event
                className="w-full"
                type="search"
                name="query"
                      ...
                      data.map(song => (
                        <Link className="w-full hover:bg-secondary/30 border border-border rounded-md p-3 flex items-center justify-between gap-3" key={song.id} href={`/${song.id}`}>
                          <div className="flex items-center gap-3">
                            <img src={song.image[2]?.url} alt={song.name} className="bg-secondary/50 w-8 h-8 rounded-md" /> {/* Use optional chaining */}
                            <p className="text-sm grid">
                              {song.name.slice(0, 40)} {song.name.length > 40 && "..."} <span className="text-muted-foreground">{song.artists.primary[0]?.name || "unknown"}</span>{" "}
