...
function ChatContent() {
  ...
  const [modelConfig, setModelConfig] = useState<ModelConfig>({ temperature: 0.7, maxOutputTokens: 2048, topP: 0.95, topK: 40 });
    ...
    if (currentSessionId) {
      const fetchMessages = async () => {
        ...
        try {
          ...
          const messagesWithDates = data.map((message: ChatMessage) => ({ ...message, createdAt: new Date(message.createdAt) }));
          setResponse(messagesWithDates);
        } catch (error) {
          console.error("Error loading messages:", error);
          if (response.length > 0) setResponse([{ id: Date.now().toString(), type: "system", content: "Failed to load messages.", createdAt: new Date() }]);
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setUserInput("");
    const userMessageObj: ChatMessage = { id: Date.now().toString(), type: "user" as const, content: userMessage, createdAt: new Date() };
    setResponse(prevResponse => [...prevResponse, userMessageObj]);
    const botMessagePlaceholderId = Date.now().toString() + "-bot";
    setResponse(prevResponse => [...prevResponse, { id: botMessagePlaceholderId, type: "bot", content: "", createdAt: new Date() }]);
    try {
      ...
      if (tokenCountHeader) {
        try {
          ...
          setTokenCount(prevCount => ({ input: tokenData.total, history: prevCount.history + tokenData.total }));
        } catch {}
      }
  ...
  const createCache = async () => {
    try {
      const res = await fetch("/api/chat/cache", {
        ...
        body: JSON.stringify({
          model: currentModel.id,
          contents: response.map(msg => ({ role: msg.type === "user" ? "user" : "model", parts: [{ text: msg.content }] })),
  ...
  const deleteCache = async (cacheName: string) => {
    try {
      await fetch("/api/chat/cache", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: cacheName }) });
      setCaches(prev => prev.filter(cache => cache.name !== cacheName));
      if (currentCache?.name === cacheName) setCurrentCache(null);
    } catch {}
  };
  return (
    <div className="flex h-screen bg-gradient-to-br from-black to-blue-950 text-gray-100 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 p-6 relative">
        <div className="flex justify-between items-center mb-6">
          <Link href="/" passHref>
            <motion.button
              className="bg-gradient-to-r from-black to-blue-950 text-white px-4 py-2 rounded-xl shadow-md border border-blue-900/50"
              ...
              <div className="space-y-4">
                ...
                {tokenCount.input > 0 && (
                  <div className="text-sm text-gray-400">
                    <p>Input Tokens: {tokenCount.input}</p> <p>History Tokens: {tokenCount.history}</p>
                  </div>
                )}
              </div>
                      ...
                      {caches.map(cache => (
                        <div key={cache.name} className="flex items-center justify-between bg-black/30 p-2 rounded-lg">
                          <div className="flex-1">
                            <p className="text-sm text-gray-300 truncate">{cache.name}</p> <p className="text-xs text-gray-500">Expires: {new Date(cache.expireTime).toLocaleString()}</p>
                          </div>
                          <div className="flex space-x-2">
                            <motion.button
          ...
          <div className="mt-6">
            <motion.div
              ...
              <label className="p-2 text-gray-400 hover:text-blue-500 cursor-pointer">
                <input type="file" onChange={handleFileUpload} className="hidden" accept=".txt,.csv,.md,.json" /> <FiUpload size={20} />
              </label>
              <input
                type="text"
                value={userInput}
              ...
              <motion.button
                ...
                className={`p-3 rounded-xl cursor-pointer ${
                  isLoading || !userInput.trim() ? "bg-gray-900 text-gray-600" : "bg-gradient-to-r from-blue-700 to-blue-950 text-white border border-blue-600/30"
                }`}
      ...
      fallback={
        <div className="flex h-screen items-center justify-center bg-gradient-to-br from-black to-blue-950 text-gray-100">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto mb-4"></div> <p className="text-lg">Loading chat...</p>
          </div>
        </div>
      }>
      <ChatContent />
