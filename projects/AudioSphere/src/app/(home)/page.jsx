...
import TrendingHero from "@/components/elements/trending";
...
export default function Page() {
  ...
  const getSongs = async (e, type) => {
    const get = await fetch(`${BASE_URL}api/search/songs?query=${e}`);
    const data = await get.json();
    if (type === "latest") setLatest(data.data.results);
    else if (type === "popular") setPopular(data.data.results);
  };
  const getAlbum = async () => {
    const get = await fetch(`${BASE_URL}api/search/albums?query=latest`);
    const data = await get.json();
